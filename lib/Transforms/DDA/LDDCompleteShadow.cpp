//===- LDDCompleteShadow.cpp - Instrument for loop data dependence --------===//
////
...
//
//
#include "LDDCompleteShadow.h"


#define DEBUG_TYPE "insert-ldd-cshadow"
...
namespace {
  ...
  class LDDCompleteShadow: public ModulePass,
  public InstVisitor<LDDCompleteShadow>{
    private:
      ...
      Function *InitLDDCompleteShadowFunction;
      Function *FiniLDDCompleteShadowFunction;
      Function *OutputLDDCShadowFunction;
      Function *AddPresentToPrecedeFunction;
    ...
    public:
      static char ID;
      LDDCompleteShadow(): ModulePass(ID){ 
        std::fstream fs;
...
char LDDCompleteShadow::ID = 0;

static RegisterPass<LDDCompleteShadow> X("insert-ldd-cshadow", "Loop Data Dependence Profiling Pass", false, false);

#if 0
INITIALIZE_PASS(LDDCompleteShadow, "insert-ldd-profiling", "Insert LDD Complete Shadow", false, false)
FunctionPass *llvm::createLDDComplteShadowPass(){
  return new LDDCompleteShadow();
}
#endif 


// Not called now. 20130922
bool LDDCompleteShadow::doInitialization(Module &M){
  ...
  M.getOrInsertFunction("__checkLoadCShadow", VoidTy, VoidPtrTy, SizeTy, VoidPtrTy, VoidPtrTy,  NULL);
  M.getOrInsertFunction("__checkStoreShadow", VoidTy, VoidPtrTy, SizeTy, VoidPtrTy,VoidPtrTy, NULL);
  M.getOrInsertFunction("__initLDDCompleteShadow", VoidTy, NULL);
  M.getOrInsertFunction("__finiLDDCompleteShadow", VoidTy, NULL);

  M.getOrInsertFunction("__initprofiling", VoidTy, NULL);
  M.getOrInsertFunction("__outputLDDCSDependence", VoidTy, NULL);
...
bool LDDCompleteShadow::runOnFunction(Function &F, int flag){


  InitLDDCompleteShadowFunction = F.getParent()->getFunction("__initLDDCompleteShadow");
  assert(InitLDDCompleteShadowFunction && "__initLDDCompleteShadow function has disappeared! \n");

  FiniLDDCompleteShadowFunction = F.getParent()->getFunction("__finiLDDCompleteShadow");
  assert(FiniLDDCompleteShadowFunction && "__finiLDDCompleteShadow function has disappeared! \n");

  OutputLDDCShadowFunction = F.getParent()->getFunction("__outputLDDCSDependence");
  assert(OutputLDDCShadowFunction && "__outputLDDCSDependence function has disappeared! \n");

  AddPresentToPrecedeFunction = F.getParent()->getFunction("addpendingtohistory");
  assert(AddPresentToPrecedeFunction && "__addpendingtohistory function has disappeared! \n");
  ...
  instrumentInit( InitLDDCompleteShadowFunction, *firstInst);
...
bool LDDCompleteShadow::runOnFunction(Function &F){

  // Check that the load and store check functions are declared.
  CheckLoadFunction = F.getParent()->getFunction("__checkLoadCShadow");
  assert(CheckLoadFunction && "__checkLoadCShadow function has disappeared!\n");

  CheckStoreFunction = F.getParent()->getFunction("__checkStoreCShadow");
  assert(CheckStoreFunction && "__checkStoreCShadow function has disappeared!\n");
  ...
  CheckLoadStackVarFunction = F.getParent()->getFunction("__checkLoadCShadowStackVar");
  assert( CheckLoadStackVarFunction  && "__checkLoadCShadowStackVar function has disappeared!\n");

  CheckStoreStackVarFunction = F.getParent()->getFunction("__checkStoreCShadowStackVar");
  assert(CheckStoreStackVarFunction &&"__checkStoreCShadowStackVar function has disappeared! \n");
...
// Insert DDA Profiling Controller Codes.
  ...
  StringRef GVarStr("LDDCSProfLoopID");
  GlobalVariable *GVar = F.getParent()->getGlobalVariable(GVarStr);
  StringRef GVarStr1("LDDCSProfLoopIter");
...
// Not need this ???
      ...
      //if (MDNode *MD = I.getMetadata("dbg"))
       ...
       // Create and Insert DDAProfilingFlag = N;
      ...
      cout << "LoopPosStr: " << LoopPosStr << endl;
       ...
       // Insert ProfilingLoopIterIter += DDABeginCurFunc;  in the loop.body
      ...
      // Insert func call: void addpendingtohistory() in the begin of loop body.
      instrumentInit( AddPresentToPrecedeFunction, LoopBody->front() ); 
      ...
      // Insert func call: void addpendingtohistory() in the begin of loop body.
      instrumentInit( AddPresentToPrecedeFunction, ExitIfTrue->front() ); 
...
#define PRINT_MODULE dbgs() << \
  "\n\n=============== Module Begin ==============\n" << M << \
"\n================= Module End   ==============\n"
bool LDDCompleteShadow::runOnModule(Module &M){
  ...
  GlobalVariable *intval = new GlobalVariable(M, type, false, GlobalVariable::AvailableExternallyLinkage , ConstantInt::getIntegerValue(type, APInt(32, 0)), Twine("LDDCSProfLoopID") );
  GlobalVariable *GIterID = new GlobalVariable(M, type, false, GlobalVariable::AvailableExternallyLinkage , ConstantInt::getIntegerValue(type, APInt(32, 0)), Twine("LDDCSProfLoopIter") );
  ...
  // Create function prototypes
  M.getOrInsertFunction("__checkLoadCShadow", VoidTy, VoidPtrTy, SizeTy, VoidPtrTy, VoidPtrTy,  NULL);
  M.getOrInsertFunction("__checkStoreCShadow", VoidTy, VoidPtrTy, SizeTy, VoidPtrTy,VoidPtrTy, NULL);

  M.getOrInsertFunction("__checkLoadCShadowStackVar", VoidTy, VoidPtrTy, SizeTy, VoidPtrTy, VoidPtrTy, SizeInt32Ty, NULL);
  M.getOrInsertFunction("__checkStoreCShadowStackVar", VoidTy, VoidPtrTy, SizeTy, VoidPtrTy,VoidPtrTy, SizeInt32Ty, NULL);
  //M.getOrInsertFunction("__initprofiling", VoidTy, VoidPtrTy, SizeTy, NULL);
  M.getOrInsertFunction("__initLDDCompleteShadow", VoidTy,  NULL);
  M.getOrInsertFunction("__finiLDDCompleteShadow", VoidTy,  NULL);
  M.getOrInsertFunction("__initprofiling", VoidTy,  NULL);
  ...
  M.getOrInsertFunction("addpendingtohistory", VoidTy,  NULL);
...
// void Check();
void 
LDDCompleteShadow::instrumentInit(  Function *Check, Instruction &I)
{
...
void 
LDDCompleteShadow::instrument(Value *Pointer, Value *AccessSize, Value* AddrPos, Value* VarName, Function *Check, Instruction &I){
  Builder->SetInsertPoint(&I); 
...
void
LDDCompleteShadow::instrumentaddnewbuffer(Function *AddNewProfilingBuffer, Value *FName, Value* LPos, Instruction &I )
{
  Builder->SetInsertPoint(&I); 
...
void 
LDDCompleteShadow::instrumentstackvar(Value *Pointer, Value *AccessSize, Value* AddrPos, Value* VarName, Value* CurFunc, Function *Check, Instruction &I){
  Builder->SetInsertPoint(&I); 
  Value *VoidPointer = Builder->CreatePointerCast(Pointer, VoidPtrTy);
  Value *PtrVarName = Builder->CreatePointerCast(VarName, VoidPtrTy);
  Value *PtrAddrPos = Builder->CreatePointerCast(AddrPos, VoidPtrTy);
...
void LDDCompleteShadow::visitLoadInst(LoadInst &LI){
...
#if 0
  // !AllocaInst: array(local/global), pointers, 
  else if( isa<PointerType>(cast<AllocaInst>(LI.getOperand(0))->getAllocatedType() ) ){
    cout << "AlllocaInst  Pointers";
    LI.dump();
    cout<< endl;
  }
#endif
...
#if 0
          //cout << "load last name: " << Var.str() << endl;
  ...
  if( !DoProfile )
    instrument(LI.getPointerOperand(), AccessSize,(Value*)GAddrPos, (Value*)GVarName,  CheckLoadFunction, LI);
  else{
    ...
    instrumentstackvar(LI.getPointerOperand(), AccessSize,(Value*)GAddrPos, (Value*)GVarName, (Value*)CurFuncLoad, CheckLoadStackVarFunction, LI);
...
void LDDCompleteShadow::visitStoreInst(StoreInst &SI){
  ...
  if( !DoProfile )
    instrument(SI.getPointerOperand(), AccessSize,(Value*)GAddrPos, (Value*)GVarName,  CheckStoreFunction, SI);
  else{
    Function *F = SI.getParent()->getParent();
    inst_iterator FirstInst = inst_begin(F); 
    Value* DDABeginCurFuncDec = &(*FirstInst);
    LoadInst *CurFuncLoad = new LoadInst(DDABeginCurFuncDec, Twine(""), &SI);
    instrumentstackvar(SI.getPointerOperand(), AccessSize,(Value*)GAddrPos, (Value*)GVarName, (Value*)CurFuncLoad, CheckStoreStackVarFunction, SI);
...
void LDDCompleteShadow::visitReturnInst(ReturnInst &I){
  ...
  instrumentInit( OutputLDDCShadowFunction, I);
#endif
...
void LDDCompleteShadow::visitAtomicRMWInst(AtomicRMWInst &I){
...
void LDDCompleteShadow::visitAtomicCmpXchgInst(AtomicCmpXchgInst &I){
  ...
  instrument(I.getPointerOperand(), AccessSize, (Value*) GAddrPos, (Value*)GVarName, CheckStoreFunction, I);
  ++AtomicsInstrumented;
}

void LDDCompleteShadow::visitMemIntrinsic(MemIntrinsic &MI){
...
void LDDCompleteShadow::visitCallInst(CallInst &CI){
  //CI.dump();
  ...
  std::string 
LDDCompleteShadow::getInstFileName(Instruction*I)
{
...
unsigned int LDDCompleteShadow::getInstLineNumber(Instruction*I)
{
  if( MDNode *Dbg = I->getMetadata(dbgKind) ){
    DILocation Loc(Dbg);
  ...
  std::string 
LDDCompleteShadow::getInstFuncName(Instruction*I)
{
#if 0
  if( MDNode *Dbg = I->getMetadata(dbgKind) ){
...
#if 0
StringRef
LDDCompleteShadow::getPointerName(Instruction &I)
{
